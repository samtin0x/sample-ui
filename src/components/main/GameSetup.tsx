...
const GameSetup = ({ onGameStart }: GameSetupProps) => {
  ...
  const handlePersonalityChange = (index: number, personality: string) => {
    setAgents((prev) =>
      prev.map((agent, i) => (i === index ? { ...agent, personality } : agent)),
    );
  };

  const handleNameChange = (index: number, name: string) => {
    setAgents((prev) =>
      prev.map((agent, i) => (i === index ? { ...agent, name } : agent)),
    );
  };
        ...
        <CardContent className="space-y-8">
          ...
          <Alert className="bg-blue-50 border-blue-200">
            <AlertDescription className="text-sm text-blue-700">
              Set the ticket price and configure each agent&apos;s name, initial
              tokens, and personality.
            </AlertDescription>
          </Alert>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          handleTokensChange(index, e.target.value)
                        }
                        className="w-full"
                        min={1}
                      />
